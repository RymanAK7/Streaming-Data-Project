...
resource "aws_iam_policy" "lambda_secrets_access" {
  ...
  path        = "/"
  description = "IAM policy for accessing Secrets Manager from a lambda"
...
resource "aws_iam_policy" "kinesis_put_record_policy" {
  name        = "Kinesis_PutRecord_Policy"
  description = "IAM policy for putting records into a Kinesis stream"
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = "arn:aws:kinesis:${var.myregion}:${var.accountId}:stream/${var.kinesis_stream_name}"
    }]
  })
}
...
resource "aws_iam_policy_attachment" "lambda_secrets_access_attachment" {
  name       = "lambda_secrets_access_attachment"
  roles      = [aws_iam_role.lambda_role.name]
  policy_arn = aws_iam_policy.lambda_secrets_access.arn
}

resource "aws_iam_role_policy_attachment" "kinesis_put_record_attachment" {
  role       = aws_iam_role.lambda_role.name
